# This script performs a Random Forest classification task, including data loading,
# preprocessing, model training with GridSearchCV, evaluation, and feature importance analysis.
# It saves performance metrics, plots (learning curves, ROC, precision-recall, confusion matrix),
# and the trained model to a specified output directory.
...
RANDOM_STATE = 42
np.random.seed(RANDOM_STATE) # Ensure reproducibility for numpy operations
TEST_SET_SIZE = 0.2
...
def main():
    ...
    parameters = {
        ...
        'classification__min_samples_leaf': [1, 3],
    ...
    def plot_roc_curve(y_true, y_proba_pred, output_dir):
        ...
        plt.grid(True, linestyle='--', alpha=0.7)
        save_plot('roc_curve.png', directory=output_dir)

    if y_proba is not None and len(label_encoder.classes_) == 2:
        plot_roc_curve(y_test, y_proba, OUTPUT_DIR)

    # Precision-Recall Curve (only for binary classification)
    def plot_precision_recall_curve(y_true, y_proba_pred, output_dir):
        prec, rec, _ = precision_recall_curve(y_true, y_proba_pred)
        ...
        plt.grid(True, linestyle='--', alpha=0.7)
        save_plot('precision_recall_curve.png', directory=output_dir)

    if y_proba is not None and len(label_encoder.classes_) == 2:
        plot_precision_recall_curve(y_test, y_proba, OUTPUT_DIR)


    # Confusion Matrix
    def plot_confusion_matrix(conf_matrix_data, labels, output_dir):
        plt.figure(figsize=FIG_SIZE_SMALL)
        disp = ConfusionMatrixDisplay(confusion_matrix=conf_matrix_data, display_labels=labels)
        ...
        plt.yticks(fontsize=TICK_FONTSIZE)
        save_plot('confusion_matrix.png', directory=output_dir)

    plot_confusion_matrix(conf_matrix, label_encoder.classes_, OUTPUT_DIR)

    # Distribution of Predicted Probabilities (only for binary classification)
    def plot_predicted_probabilities(y_proba_pred, output_dir):
        plt.figure(figsize=FIG_SIZE_SMALL)
        sns.histplot(y_proba_pred, bins=30, kde=True, color=HIST_COLOR_GREEN, edgecolor='darkgreen')
        plt.xlabel('Predicted Probability (Class 1)', fontsize=LABEL_FONTSIZE)
        plt.ylabel('Frequency', fontsize=LABEL_FONTSIZE)
        plt.xticks(fontsize=TICK_FONTSIZE)
        plt.yticks(fontsize=TICK_FONTSIZE)
        plt.grid(True, linestyle='--', alpha=0.7)
        save_plot('predicted_probabilities_distribution.png', directory=output_dir)

    if y_proba is not None and len(label_encoder.classes_) == 2:
        plot_predicted_probabilities(y_proba, OUTPUT_DIR)
    ...
    def plot_gini_importance(gini_df, top_n, output_dir):
        top_df = gini_df.head(top_n)
        fig_h = max(6, len(top_df) * 0.35)
        plt.figure(figsize=(FIG_SIZE_SMALL[0], fig_h))
        sns.barplot(x='Gini_Importance', y='Feature', data=top_df, hue='Type', dodge=False, palette={'ART': 'mediumseagreen', 'TRT': 'darkseagreen'})
        ...
        save_plot(f'top_{top_n}_gini_importances.png', directory=output_dir)

    plot_gini_importance(gini_importances_df, TOP_N_FEATURES_DISPLAY, OUTPUT_DIR)

    # Plot Top N Permutation Importances
    def plot_permutation_importance(perm_df, top_n, output_dir):
        top_df = perm_df.head(top_n)
        fig_h = max(6, len(top_df) * 0.35)
        plt.figure(figsize=(FIG_SIZE_SMALL[0] + 1, fig_h))
        y_coords = np.arange(len(top_df))
        color_art = '#228B22'  # ForestGreen
        color_trt = '#00CED1'  # DarkTurquoise
        plt.barh(y_coords, top_df['Permutation_Importance_Mean'],
                 xerr=top_df['Permutation_Importance_Std'],
                 align='center',
                 color=[color_art if t == 'ART' else color_trt for t in top_df['Type']],
                 ecolor='gray', capsize=4)
        plt.yticks(y_coords, top_df['Feature'])
        art_patch = mpatches.Patch(color=color_art, label='ART')
        trt_patch = mpatches.Patch(color=color_trt, label='TRT')
        plt.legend(handles=[art_patch, trt_patch], fontsize=LEGEND_FONTSIZE - 2)
        plt.xlabel('Permutation Importance (Mean ± SD)', fontsize=LABEL_FONTSIZE)
        plt.ylabel('Feature', fontsize=LABEL_FONTSIZE)
        plt.gca().invert_yaxis()
        plt.xticks(fontsize=TICK_FONTSIZE)
        plt.yticks(fontsize=TICK_FONTSIZE)
        save_plot(f'top_{top_n}_permutation_importances.png', directory=output_dir)

    plot_permutation_importance(perm_importances_df, TOP_N_FEATURES_DISPLAY, OUTPUT_DIR)
