...
flowchart TD;
    ...
    classification --> classFiles[Classification
        CatBoost.py
        Catboost_Validation.py
        RandomForest.py
        RandomForest_Validation.py]

    rests --> restFiles[Rests
        CDF_Plot.py
    ...
    requirements --> reqFiles[Requirements
        requirements.txt
        r_requirements.txt]

style root fill:#f96,stroke:#333,stroke-width:4px
style scripts fill:#f96,stroke:#333,stroke-width:2px
...
2. **Preprocessing**
   ...
   - Background removal
   - [RootPainter](https://github.com/Abe404/root_painter) segmentation (Dice score approaching 1)

3. **Trait Extraction**
   - Traditional trait calculation (23 TRTs via [RhizoVision Explorer](https://github.com/rootphenomics/RhizoVisionExplorer))
   - Algorithmic trait computation (27 ARTs via 9 algorithms)
   - Spatial coordinate analysis using [PlantCV](https://github.com/danforthcenter/plantcv)
...
## 🙏 Acknowledgments
Special thanks to [Adam Dimech](https://github.com/AdamDimech), the Docker community, and ML framework developers.
